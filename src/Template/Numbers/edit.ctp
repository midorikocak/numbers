<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $number->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $number->id)]
            )
        ?></li>
...
<div class="numbers form large-9 medium-8 columns content">
    ...
    <fieldset>
        <legend><?= __('Edit Number') ?></legend>
        <?php
            $options = ['-1' => 'Negative', '0' => 'Neutral', '1'=>'Positive'];
            echo $this->Form->select('status', $options, ['label'=>'Feedback']);
